...
3. The prompt's "Read first" list (specs, memory files, AGENTS.md sections it names).
...
4. If the session runs long, **keep status current before any compaction**: write progress into
   the plan doc or project AGENTS.md so a resumed session starts warm (Eston's standing
