...
disagrees with even AGENTS.md, the code is the current truth and the doc has
...
|---|---|---|---|
| 1 | `AGENTS.md` | Agent instructions: stack, branch model, seam rule, key patterns, routing, deployment | Current. One known gap: its routing block omits the live `/mandate/:communityId/:mandateId/*` route (verified in `src/App.tsx:123`) |
...
| — | `AGENTS.md` | Explicitly defers wholly to AGENTS.md ("Don't duplicate project guidance here") | Never edit it with guidance; edit AGENTS.md |
...
| `ARCHITECTURE.md` (line ~11) + `docs/LANES.md` route map | "Default landing page at `/stage/problem`" | `/` → HomeView (first-run users → `/welcome`). AGENTS.md routing is correct |
...
| `AGENTS.md` routing block | (omission) no `/mandate/...` route listed | Route exists at `src/App.tsx:123` |
...
| During a long multi-task session | Compact and save implementation status to project `AGENTS.md` | Eston's standing preference (recorded Apr 2026) so interrupted sessions resume cleanly |
...
Paste into a fresh Codex session in the Communities2 repo (branch `ui`). <One-paragraph
